...
describe Neoid::ModelAdditions do
  context "promises" do
    it "should run scripts in a batch and return results" do
      Neoid.connection.batch do |batch|
    ...
    it "should run scripts in a batch with batch_size and flush batch when it's full" do
      Neoid.connection.batch(batch_size: 3) do |batch|
        (0...9).each do |i|
    ...
    it "should run scripts in a batch with batch_size and return all results" do
      Neoid.connection.batch(batch_size: 2) do |batch|
        (1..6).each do |i|
          batch << [:execute_script, i.to_s]
        end
      end.then do |results|
    ...
    it "should return results then process them" do
      node_1 = Neoid.connection.db.create_node
      node_2 = Neoid.connection.db.create_node
      rel = Neoid.connection.db.create_relationship(:related, node_1, node_2)
  ...
  context "relationships" do
    ...
    it "should not execute until batch is done" do
      ...
      res = Neoid.connection.batch do |batch|
        user.like! movie
